...
class MS5837(object):   
    ...
    def __init__(self, model=MODEL_30BA, bus=1):
        self._model = model
        self._MS5837_ADDR             = 0x76  
        self._MS5837_RESET            = 0x1E
        self._MS5837_ADC_READ         = 0x00
        self._MS5837_PROM_READ        = 0xA0
        self._MS5837_CONVERT_D1_256   = 0x40
        self._MS5837_CONVERT_D2_256   = 0x50
        
        try:
            self.mega_i2c = machine.I2C(1, scl=machine.Pin(3), sda=machine.Pin(2))
        except:
            print('no-dept-sensor-init')
            self.mega_i2c = None
    ...
    def init(self):
        if not self.mega_i2c:
            "No bus!"
            return False
        
        self._C = []
        
        self.mega_i2c.writeto(self._MS5837_ADDR, bytes([self._MS5837_RESET]))

        for i in range(7):
            c = self.mega_i2c.readfrom_mem(self._MS5837_ADDR, self._MS5837_PROM_READ + 2*i, 2)
        ...
        if crc != self._crc4(self._C):
            print("PROM read error, CRC777 failed!")
            return False
    ...
    def read(self, oversampling=OSR_8192):
        ...
        self.mega_i2c.writeto(self._MS5837_ADDR, bytes([self._MS5837_CONVERT_D1_256 + 2*oversampling]))
    
        sleep(2.5e-6 * 2**(8+oversampling))

        d = self.mega_i2c.readfrom_mem(self._MS5837_ADDR, self._MS5837_ADC_READ, 3)
        d = dec_binary(d)
        print(d)
        ...
        self.mega_i2c.writeto(self._MS5837_ADDR, bytes([self._MS5837_CONVERT_D2_256 + 2*oversampling]))
